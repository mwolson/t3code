...
import { mobileThreadSnapshotLoaderLayer } from "./mobileThreadSnapshotHttp.ts";
import { ThreadSnapshotLoader } from "./threadSnapshotHttp.ts";

const TARGET = new PrimaryConnectionTarget({
  environmentId: "environment-mobile-bounded" as never,
  label: "Mobile bounded",
...
const THREAD_ID = ThreadId.make("thread-mobile-bounded");
...
describe("mobileThreadSnapshotLoader", () => {
  it.effect("falls back to full HTTP snapshot when bounded returns a plain route 404", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        Layer.provide(mobileThreadSnapshotLoaderLayer, remoteHttpClientLayer(fetchFn)),
      ),
    );
  });
