...
/**
 * Load a mobile-oriented bounded thread snapshot over HTTP.
 */
...
/**
 * Mobile-only ThreadSnapshotLoader. Seeds from the bounded HTTP snapshot when
 * available. Older servers that lack the bounded route (plain/generic 404) fall
 * back to the existing full HTTP thread snapshot. Structured
 * EnvironmentResourceNotFoundError from either endpoint still means missing.
 * Transient failures report `unavailable` so the socket path remains a last
 * resort for connectivity issues.
 */
export const mobileThreadSnapshotLoaderLayer: Layer.Layer<
  ThreadSnapshotLoader,
  never,
