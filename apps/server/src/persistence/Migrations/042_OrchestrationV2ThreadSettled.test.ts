...
MigrationLayer("042_OrchestrationV2ThreadSettled", (it) => {
  it.effect(
    "upgrades a migration-041 V2 thread payload without settlement fields, then persists first settle",
    () =>
      Effect.gen(function* () {
        const sql = yield* SqlClient.SqlClient;

        // Stop before the V2 settled-column migration. Seed a real pre-042
        ...
        // Historical migration-041 shape: no settledOverride / settledAt keys.
        const pre042PayloadJson = yield* Schema.encodeUnknownEffect(Schema.UnknownFromJsonString)({
          createdBy: "user",
          creationSource: "web",
          id: threadId,
          projectId,
          title: "Pre-042 V2 thread",
        ...
        yield* Effect.gen(function* () {
          ...
          yield* store.apply({
            id: EventId.make("event:migration-042-first-settle"),
            type: "thread.settled",
            threadId,
