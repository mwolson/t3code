...
export interface EventSinkV2Shape {
   ...
   * expected run ordinal. Used for late post-terminal provider_thread updates
   * so a completed run cannot clobber a newer run that already claimed the
   * thread (lastRunOrdinal advanced).
   */
      ...
      const result = yield* sql.withTransaction(
        Effect.gen(function* () {
          const rows = yield* sql<{
            ...
            SELECT last_run_ordinal
            FROM orchestration_v2_projection_provider_threads
            WHERE provider_thread_id = ${input.providerThreadId}
            LIMIT 1
          `;
          const current = rows[0];
          if (current === undefined || current.last_run_ordinal !== input.expectedLastRunOrdinal) {
            return {
              committed: false as const,
              storedEvents: [] as ReadonlyArray<OrchestrationV2StoredEvent>,
            };
