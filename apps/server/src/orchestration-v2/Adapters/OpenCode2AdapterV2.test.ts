...
import { EnvironmentId, ProviderInstanceId, ProviderThreadId, ThreadId } from "@t3tools/contracts";
...
import {
  ...
  openCode2PostSettleContinuationRequest,
...
describe("OpenCode 2 post-settle wake classification", () => {
  it("keeps native wake detail while using generic buffered continuation text", () => {
    const request = openCode2PostSettleContinuationRequest({
      threadId: ThreadId.make("thread:root"),
      providerThreadId: ProviderThreadId.make("provider-thread:root"),
      detail: "background child fixture",
    });
