/**
 * OpenCode 2.x orchestration adapter.
 *
 * This is the live OpenCode stack. OpenCode 1.x is retired. The built-in
 * `opencode` identity and leftover `opencode2` instances share this adapter.
 * Concretely,
 *
...
import {
  makeOpenCode2TurnUsage,
  openCode2TurnTokenUsage,
  type OpenCode2TurnUsage,
} from "./openCodeTurnUsage.ts";
...
import { applyOpenCodeProviderEnvironment } from "../../provider/OpenCodeProviderEnvironment.ts";
...
interface ActiveOpenCode2Turn {
  readonly usage: OpenCode2TurnUsage;
...
type OpenCodeRuntimeRequestSettlement = {
...
interface OpenCodeRuntimeRequestProjection {
...
interface PendingOpenCode2Request extends OpenCodeRuntimeRequestProjection {
  ...
  responseSettlement: OpenCodeRuntimeRequestSettlement | null;
...
interface SettledOpenCode2RequestProjection extends OpenCodeRuntimeRequestProjection {
  authoritativeCancellation: boolean;
...
export function openCode2TokenUsage(
  input: unknown,
  includeZero = false,
): OpenCode2TokenUsage | null {
  ...
  return includeZero || Object.values(usage).some((value) => value > 0) ? usage : null;
}
...
export function openCode2LastErrorAt(input: {
  ...
  // A status refresh is not a new occurrence, including legacy errors with no timestamp.
  return input.previousErrorAt ?? null;
}
...
export const openCode2PendingWorkForSession = Effect.fnUntraced(function* (input: {
  readonly sessionID: string;
  readonly pending: Effect.Effect<ReadonlyArray<SessionPendingInfo>, OpenCodeRuntimeError>;
  readonly shells: Effect.Effect<ReadonlyArray<ShellInfoV2>, OpenCodeRuntimeError>;
}) {
...
export function makeOpenCode2ProtocolLogger(input: {
  ...
  const driver = input.driver ?? OPENCODE_PROVIDER;
  return (event) =>
    Effect.gen(function* () {
...
function protocolError(
  detail: string,
  payload?: unknown,
  driver: ProviderDriverKind = OPENCODE_PROVIDER,
): ProviderAdapterProtocolError {
...
export function openCode2SessionSelectionParameters(
  ...
  const variant = normalizeOpenCodeVariant(
...
export function resolveOpenCode2SessionAgent(
  ...
  const explicit = explicitAgent === OPENCODE_AUTO_AGENT ? undefined : explicitAgent;
 ...
 * `retryEmptyOpenCodeInventory`, additionally retrying failed fetches, which
...
export function planOpenCode2VariantAlignment(input: {
  ...
  const clamp = clampOpenCode2Variant(
    normalizeOpenCodeVariant(input.rawVariant),
    input.knownVariants,
  );
  return {
    ...clamp,
...
function nativeThreadId(providerThread: OrchestrationV2ProviderThread): string {
  ...
  if (nativeId === null || nativeId === undefined) {
    throw protocolError(
      `Provider thread ${providerThread.id} has no OpenCode 2 session id`,
      undefined,
...
export function unwrapOpenCode2Data<A>(
  ...
  result: unknown,
): Effect.Effect<NonNullable<A>, OpenCodeRuntimeError> {
...
export const interruptOpenCode2Descendants = Effect.fnUntraced(function* (input: {
  ...
  readonly list: (parameters: {
    ...
    OpenCodeRuntimeError
  >;
  readonly interrupt: (sessionID: string) => Effect.Effect<unknown, OpenCodeRuntimeError>;
}) {
  ...
  while (pending.length > 0) {
    ...
    do {
      const listed = yield* input
        .list({ parentID, ...(cursor === undefined ? {} : { cursor }) })
        .pipe(
          Effect.catchIf(
            (error) => openCode2ClientRemovalAlreadyMissing(error.cause),
            () => Effect.succeed({ data: [], cursor: { next: null } }),
          ),
          Effect.result,
        );
      ...
      for (const child of children) {
        ...
        const stopped = yield* input.interrupt(child.id).pipe(
          Effect.catchIf(
            (error) => openCode2ClientRemovalAlreadyMissing(error.cause),
            () => Effect.void,
          ),
...
export function removeOpenCode2Session(
  sessionID: string,
  request: Effect.Effect<unknown, OpenCodeRuntimeError>,
...
export function makeOpenCodeAdapterV2(options: OpenCodeAdapterV2Options): ProviderAdapterV2Shape {
  ...
  const driver = options.driver ?? OPENCODE_PROVIDER;
  ...
  const protocolError = (detail: string, payload?: unknown): ProviderAdapterProtocolError =>
    new ProviderAdapterProtocolError({
      driver,
      detail,
      ...(payload === undefined ? {} : { payload }),
    });
  const providerRef = (nativeId: string, strength: "strong" | "weak" = "strong") =>
    ({
      driver,
      nativeId,
      strength,
  ...
  return ProviderAdapterV2.of({
    ...
    openSession: Effect.fn("OpenCode2AdapterV2.openSession")(
      function* (input: ProviderAdapterV2OpenSessionInput) {
        ...
        const selfSpawning =
          !(options.settings.serverUrl?.trim() ?? "") && connection.external !== true;
        ...
        const lastEmptyPendingWorkProbeAtMs = new Map<string, number>();
        ...
        const sdkCall = <A>(
          method: OpenCodeRuntimeOperation,
          payload: unknown,
        ...
        const emitProviderTurn = (
          ...
          const providerTurn: OrchestrationV2ProviderTurn = {
            ...
            ...(completedAt === null
              ? {}
              : { turnTokenUsage: openCode2TurnTokenUsage(turn.usage, status === "completed") }),
          };
        ...
        const bindSubagentChild = Effect.fnUntraced(function* (
          ...
          const childVariant = normalizeOpenCodeVariant(nativeSession.model?.variant);
        ...
        const runtimeRequestTurnItem = (
          pending: OpenCodeRuntimeRequestProjection,
          status: OrchestrationV2TurnItem["status"],
          completedAt: DateTime.Utc | null,
        ...
        const respondWithRuntimeRequestSettlement = Effect.fnUntraced(function* <A, E, R>(
          ...
          const exit = yield* Effect.exit(response.pipe(Effect.timeout("10 seconds")));
        ...
        const failActiveTurns = Effect.fnUntraced(function* (
          ...
          yield* updateProviderSession("error", detail);
          if (nativeStreamFailure !== null) yield* Queue.end(events);
        });
        ...
        const handleEvent = Effect.fnUntraced(function* (
          ...
          if (eventSessionId !== undefined) {
            ...
            if (
              ...
              if (
                (wire.type === "session.step.ended" || wire.type === "session.step.failed") &&
                (held.turn.executionStarted || held.turn.parts.size > 0)
              ) {
                const data = openCode2WireData(wire);
                const stepId = recordString(data, "assistantMessageID");
                const tokens = openCode2TokenUsage(data, true);
                if (stepId !== undefined && tokens !== null) {
                  held.turn.usage.steps.set(stepId, {
                    input: tokens.input,
                  ...
                  held.turn.usage.complete = false;
                }
              }
        ...
        const eventPump = Effect.gen(function* () {
          ...
          while (!abortController.signal.aborted) {
            if (nativeStreamFailure !== null) return;
            ...
            if (abortController.signal.aborted || nativeStreamFailure !== null) return;
            for (const state of threads.values()) {
        ...
        const registerThread = (
          ...
          if (existing !== undefined) {
            ...
            if (nativeSession.model !== undefined) {
              ...
              existing.boundVariant = normalizeOpenCodeVariant(nativeSession.model.variant) ?? null;
            }
        ...
        const submitSessionPrompt = Effect.fnUntraced(function* (input: {
          readonly sessionID: string;
          readonly text: string;
          readonly files?: ReturnType<typeof toOpenCode2FileAttachments>;
          readonly delivery?: "steer" | "queue";
        }) {
          const match = input.text.trim().match(/^\/([^\s/]+)(?:\s+([\s\S]*))?$/);
          // Compact already follows the existing OC2 prompt/compaction lifecycle.
          if (match && match[1] !== "compact") {
            const commands = yield* loadOpenCodeCommands(client, cwd).pipe(
              Effect.timeout("10 seconds"),
              Effect.orElseSucceed(() => []),
            );
            if (commands.some((command) => command.name === match[1])) {
              const payload = { ...input, command: match[1]!, text: match[2] ?? "" };
              yield* sdkCall("session.command", payload, (signal) =>
                client.session.command(payload, { signal }),
              ).pipe(Effect.timeout("10 seconds"));
              return undefined;
            }
          }
          return yield* sdkCall("session.prompt", input, (signal) =>
        ...
        const inspectPendingBackgroundWork = Effect.fnUntraced(function* (
          ...
          const sessionID = state.nativeSessionId;
          const hasWork = yield* openCode2PendingWorkForSession({
            sessionID,
          ...
          if (hasWork) lastEmptyPendingWorkProbeAtMs.delete(sessionID);
          else lastEmptyPendingWorkProbeAtMs.set(sessionID, yield* Clock.currentTimeMillis);
          return hasWork;
        });
        ...
        const recentlyProbedEmpty = (sessionID: string, nowMs: number) => {
        ...
        const hasPendingBackgroundWorkForState = (state: OpenCode2ThreadState) =>
          Effect.gen(function* () {
            ...
            return yield* inspectPendingBackgroundWork(state).pipe(
        ...
        const waitForT3Mcp = Effect.fnUntraced(function* () {
          ...
          for (let attempt = 0; attempt < 50; attempt++) {
            ...
            if (lastStatus !== "missing" && lastStatus !== "pending") {
              yield* Effect.logWarning(
                "OpenCode 2 T3 MCP did not connect; continuing without it.",
                {
                  lastStatus,
                  provider: driver,
                },
          ...
          yield* Effect.logWarning("OpenCode 2 T3 MCP connect timed out; continuing without it.", {
            lastStatus,
            provider: driver,
          });
        });
        ...
        const runtimeSession: ProviderAdapterV2SessionRuntime = {
          ...
          startTurn: (turnInput) =>
            Effect.gen(function* () {
              if (nativeStreamFailure !== null)
                return yield* protocolError(
                  "OpenCode event stream has failed; reconnect before starting another turn.",
                );
              ...
              // IDs can yield while the event pump exhausts recovery.
              if (nativeStreamFailure !== null)
                return yield* protocolError(
                  "OpenCode event stream has failed; reconnect before starting another turn.",
                );
              if (state.activeTurn !== null)
                return yield* protocolError("OpenCode thread already has an active turn.");
              const turn: ActiveOpenCode2Turn = {
                isRoot: true,
              ...
              const prompted = yield* submitSessionPrompt({ sessionID, ...payload }).pipe(
                Effect.tapError(finalizeFailedTurn),
              );
          ...
          steerTurn: (steerInput) =>
            Effect.gen(function* () {
              ...
              yield* submitSessionPrompt({ sessionID, ...payload, delivery: "steer" });
            }).pipe(
              Effect.mapError(
                (cause) =>
          ...
          interruptTurn: (interruptInput) =>
            Effect.gen(function* () {
              ...
              const descendants = yield* interruptDescendants(sessionID).pipe(Effect.exit);
              const checkStop = Effect.gen(function* () {
                if (Exit.isFailure(descendants)) return yield* Effect.failCause(descendants.cause);
                if (!interruptRequestConfirmed || !shellRemovalConfirmed)
                  return yield* protocolError(
                    "OpenCode interruption was not confirmed for every native target.",
                  );
              });
              if (
              ...
              while (true) {
                if (turn.finalized || state.activeTurn !== turn) return yield* checkStop;
                const now = yield* Clock.currentTimeMillis;
            ...
            Effect.gen(function* () {
              ...
              if (pending.questions !== undefined) {
                ...
                yield* respondWithRuntimeRequestSettlement(
                  ...
                  sdkCall("session.question.reply", { sessionID, requestID, answers }, (signal) =>
                    client.form.reply(
                      {
                        sessionID,
                        formID: requestID,
              ...
              yield* respondWithRuntimeRequestSettlement(
                ...
                sdkCall("session.permission.reply", { sessionID, requestID, reply }, (signal) =>
                  client.permission.reply({ sessionID, requestID, reply }, { signal }),
                ),
              );
            }).pipe(
              Effect.mapError(
          ...
          rollbackThread: (rollbackInput) =>
            Effect.gen(function* () {
              ...
              if (boundaryMessageId !== undefined) {
                // The checkpoint service, not native rewind, owns filesystem restoration.
                ...
                const fork = yield* sdkCall("session.fork", parameters, () =>
                  postSessionFork(parameters),
                ).pipe(
                  Effect.flatMap((response) =>
                    unwrapOpenCode2Data<SessionInfoV2>("session.fork", response),
                  ),
                );
                const retained = yield* sdkCall("message.list", { sessionID: fork.id }, () =>
                  client.message.list({ sessionID: fork.id }),
                ).pipe(
                  Effect.flatMap((response) =>
                    unwrapOpenCode2Data<Array<SessionMessageInfo>>("message.list", response),
                  ),
                );
                ...
                yield* installT3OrchestrationInstructions(fork.id);
                registerThread(fork, retainedThread).appThread = state?.appThread ?? null;
              }
...
export function createOpenCodeAdapterV2Driver(
  driverKind: ProviderDriverKind,
): ProviderAdapterDriver<OpenCode2Settings, OpenCodeAdapterV2DriverEnv> {
  return {
    driverKind,
...
export const OpenCodeAdapterV2Driver = createOpenCodeAdapterV2Driver(OPENCODE_PROVIDER);
