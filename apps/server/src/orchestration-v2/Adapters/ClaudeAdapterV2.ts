...
//
// Failures terminalize as before. The two aborted reasons are accepted on error
// results too, since that is exactly how `aborted_streaming` arrives; a failure
// that ends the query instead is bounded by the query-exit finalize.
    ...
    openSession: Effect.fn("ClaudeAdapterV2.openSession")(
      function* (input: ProviderAdapterV2OpenSessionInput) {
        ...
        const handleSdkMessage = Effect.fnUntraced(function* (input: {
          ...
          // A zero-turn task-notification-origin result is lifecycle debris:
          // it must not finalize a normal turn or supply fallback assistant
          // text. Provider continuation turns still consume it when draining
          // buffered wake messages.
          if (
