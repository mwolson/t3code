    ...
    openSession: Effect.fn("AcpAdapterV2.openSession")(
      function* (input: ProviderAdapterV2OpenSessionInput) {
        ...
        const setBackgroundTaskRunning = (taskId: string, running: boolean) =>
          Ref.update(runningBackgroundTaskIds, (current) => {
        ...
        const bufferPostSettleWake = Effect.fnUntraced(function* (
          ...
          // Per-event agent commentary while a monitor is still streaming must
          // buffer, not offer: each burst would otherwise reopen a synthetic
          // "Background task completed." run. The commentary drains into the
          // single continuation that opens once the monitor actually ends.
          if (
            (update.sessionUpdate === "agent_message_chunk" ||
              update.sessionUpdate === "agent_thought_chunk") &&
          ...
          if (context === null) {
            ...
            if (
              ...
              if (lateBackgroundMutation !== undefined) {
                yield* setBackgroundTaskRunning(
                  lateBackgroundMutation.taskId,
                  lateBackgroundMutation.status === "running",
                );
              }
            ...
            case "user_message_chunk":
              if (update.content.type === "text" && flavor.extractBackgroundToolMutation) {
                ...
                if (mutation !== undefined) {
                  ...
                  yield* setBackgroundTaskRunning(mutation.taskId, mutation.status === "running");
                }
              }
              break;
            default: {
