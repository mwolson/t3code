      ...
      const checkProvider = checkOpenCodeProviderStatus(
        ...
        Effect.flatMap((provider) =>
          DateTime.now.pipe(
            Effect.map((now) => ({
              ...provider,
              // 18999 exposes neither account quotas nor the OC1 auth.json credential contract.
              // Do not display a different local OC1 account's Go quota for the host daemon.
