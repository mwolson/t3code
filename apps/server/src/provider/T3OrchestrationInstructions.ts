...
- A delegated task/subagent is child work owned by the current thread. Prefer the current provider's native subagent tools for same-provider parallel work when available. Use \`delegate_task\` for cross-provider work, when native delegation is unavailable, or when the user explicitly requests T3-owned child tasks. Call \`orchestrator_capabilities({})\` for provider summaries, pass \`providerInstanceId\` to page that provider's models with \`modelCursor\`, and request an exact \`model\` with \`includeModelOptions: true\` only when its option descriptors are needed. Delegated children always inherit the caller's project. Retain each returned \`taskId\`, and use \`task_status\` or \`task_cancel\` to manage it. The returned \`childThreadId\` is backing storage for the subagent; do not replace delegation with ordinary thread creation.
...
\`t3_thread_launch\` is the single-thread launch tool. Use \`create_threads\` only for a batch of threads intentionally sharing the caller's checkout: it inherits the caller's project, branch, and worktree unless an entry supplies \`projectDirectory\`, an absolute or \`~/\` path to another known project's root checkout. Unknown or relative paths are rejected. \`t3_thread_start\` offers the same project selection for one prompt. Neither tool selects worktrees; use launch for that. Asking an agent to run \`git worktree add\` or \`cd\` in its prompt does not update T3's thread binding. Select the workspace in the launch call instead. \`t3_worktree_handoff\` moves the calling thread, not another thread, and cannot move a thread already attached to a worktree.
