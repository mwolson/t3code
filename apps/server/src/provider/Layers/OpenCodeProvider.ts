 ...
 * @internal exported for tests
 */
export function openCodeNextBuild(version: string): number | null {
    ...
    Effect.gen(function* () {
      ...
      const [inventory, healthResponse] = yield* Effect.all(
        [
          ...
          runOpenCodeSdk("health.get", () => client.health.get()),
        ],
        { concurrency: "unbounded" },
      );
...
export const listOpenCodeSkillsForDirectory = Effect.fn("listOpenCodeSkillsForDirectory")(
  ...
  ): Effect.fn.Return<ReadonlyArray<ServerProviderSkill>, never, OpenCodeRuntime> {
...
export const listOpenCodeCommandsForDirectory = Effect.fn("listOpenCodeCommandsForDirectory")(
  function* (settings: OpenCode2Settings, cwd: string, environment: NodeJS.ProcessEnv) {
    ...
    return yield* Effect.gen(function* () {
      const server = yield* runtime.connectToOpenCodeServer({ ...settings, environment });
      const client = runtime.createOpenCodeSdkClient({
        baseUrl: server.url,
        directory: cwd,
        serverPassword: server.password,
...
export const checkOpenCodeProviderStatus = Effect.fn("checkOpenCodeProviderStatus")(function* (
  ...
  const build = openCodeNextBuild(version);
