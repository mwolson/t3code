...
import { parseOpenCodeModelSlug, openCodeRuntimeErrorDetail } from "../provider/opencodeRuntime.ts";
...
function isOpenCode2ModelStartupError(error: TextGenerationError): boolean {
  return (
    OpenCode2Runtime.OpenCode2RuntimeError.is(error.cause) &&
    error.cause.detail.startsWith("Model unavailable:")
  );
}
...
export const makeOpenCode2TextGeneration = Effect.fn("makeOpenCode2TextGeneration")(function* (
  ...
  const acquireSharedServer = (
    ...
    sharedServerMutex.withPermit(
      Effect.gen(function* () {
        yield* cancelIdleCloseFiber();
        if (sharedServerState.server !== null) {
          sharedServerState.activeRequests += 1;
          return sharedServerState.server;
        }
                  ...
                  .pipe(
                    ...
                    Effect.mapError(
                      (cause) =>
                        new TextGenerationError({
                          operation,
                          detail: `OpenCode 2 server startup failed: ${openCodeRuntimeErrorDetail(cause)}`,
                          cause,
                        }),
                    ),
                  ),
    ...
    OpenCode2Runtime.runOpenCode2Sdk(method, call).pipe(
      Effect.mapError(
        (cause) =>
          new TextGenerationError({
            operation,
            detail: `OpenCode 2 ${method} request failed: ${cause.detail}`,
            cause,
          }),
      ),
    );
